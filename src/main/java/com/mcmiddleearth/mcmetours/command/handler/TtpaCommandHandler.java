...
public class TtpaCommandHandler extends AbstractCommandHandler implements TabExecutor {

    String name;

    public TtpaCommandHandler(String name, CommandDispatcher<CommandSender> dispatcher){
        super(name);
        this.name = name;
    }
    ...
    protected HelpfulLiteralBuilder createCommandTree(HelpfulLiteralBuilder helpfulLiteralBuilder) {
        helpfulLiteralBuilder.executes(context -> sendInfoMessage(context.getSource(),"TestInfo"));
    ...
    private int sendInfoMessage(McmeCommandSender sender, String text){
        PluginData.getMessageUtil().sendInfoMessage((ProxiedPlayer) sender,text);
        return 0;
    }
